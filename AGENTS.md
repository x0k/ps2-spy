...
6. ~~**Extract repeated Fallback→Cache pattern into generic factory** — `population_loader.go`, `world_population_loader.go`, `alerts_loader.go` are near-identical (~170 lines). Single generic factory cuts ~140 lines.~~ **DONE** — Created `loader/fallback_cache.go` with `SimpleFallbackCache[T]` and `KeyedFallbackCache[K,T]` generic types; moved `Query[K]` struct to `loader/query.go`. Deleted 4 files from `discord/commands/`. See commit `c688fbb`.
